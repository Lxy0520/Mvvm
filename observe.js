class Dep{
    ...
    addFunc(obj){
        this.listenFunc.push(obj)//添加watcher实例
    }
    changeWatch(){
...
class Watcher{
    constructor(data,key,callback){
        Dep.target=this//每一次实例watcher的时候，均会把当前实例赋值给Dep的target静态属性
        this.data=data
        this.key=key
...
class Mvvm{
    ...
    compiler(node){
        if(node.nodeType===1){
            ...
            let contentVal=node.textContent.indexOf('{{') > -1 && node.textContent.split('{{')[1].split('}}')[0]//替换文本内容并去掉{{}}
